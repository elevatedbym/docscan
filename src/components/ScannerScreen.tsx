  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || `Greška (${response.status})`);
      }
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-md mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-slate-900">DocSkan</span>
          </div>
          <button
            onClick={signOut}
        ...
        {limitReached && (
          <div className="mb-6 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-amber-900 text-sm">Pretplatite se za dalje korišćenje</p>
        ...
        {imagePreview && !limitReached && (
          <div className="space-y-4">
            <div className="relative rounded-2xl overflow-hidden border border-slate-200 bg-white">
              <img src={imagePreview} alt="Dokument" className="w-full max-h-72 object-contain bg-slate-50" />
              <button
                onClick={handleReset}
            ...
            {status === "analyzing" && (
              <div className="flex flex-col items-center justify-center py-8">
                ...
                <p className="text-xs text-slate-400 mt-1">Prevođenje na {LANGUAGES.find(l => l.code === selectedLanguage)?.label}</p>
              </div>
            )}
            ...
            {status === "done" && result && (
              <div className="space-y-4">
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5">
                  ...
                  <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{result.translated_text}</p>
                </div>
          ...
          <div className="mt-8">
            ...
            {showHistory && (
              <div className="space-y-2">
                {scans.map((scan) => (
                  <div key={scan.id} className="bg-white rounded-xl border border-slate-200 p-3 flex gap-3">
                    <img src={scan.image_url} alt="" className="w-14 h-14 rounded-lg object-cover bg-slate-100 shrink-0" />
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-slate-600 line-clamp-2">{scan.summary || "Nema sažetka"}</p>
                    </div>
                  </div>
                ))}
              </div>
